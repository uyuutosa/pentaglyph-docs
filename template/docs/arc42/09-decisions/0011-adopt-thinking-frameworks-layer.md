...
### Option B — Bind as **operational conventions under `design-guide/`**, in a dedicated sub-directory

Add `docs/design-guide/thinking-frameworks/` as a new sub-directory, alongside existing conventions (`version-control.md`, `ai-augmented-pr.md`, `code-tours.md`, the `bdd-workflow.md` / `tdd-workflow.md` / `dev-cycle.md` process bindings). Each framework gets a 1-2 page binding file with: authoritative source URL, when-to-use, when-not-to-use, 1-2 worked examples.

- **Pros**: structurally honest — these are operational conventions, same category as the existing `design-guide/` entries
...
Adopt only `5-minto-pyramid.md` under `design-guide/business-writing.md`. Defer MECE / Issue Tree / Hypothesis-Driven / 5 Whys / First Principles / 80-20 / 2x2 / OODA to ad-hoc later bindings.
...
**Choose Option B: bind as operational conventions under `docs/design-guide/thinking-frameworks/`.**
...
5. Existing `design-guide/` already has analogous bindings (`bdd-workflow.md`, `tdd-workflow.md`) — pattern is proven
...
- [`docs/design-guide/_binding-a-new-process.md`](../../design-guide/_binding-a-new-process.md): ✅ each framework file follows the 6-section template adapted for thinking-frameworks (source / when / when-not / example / failures / related)
...
- [ ] Add `templates/` reference: postmortem template should reference `2-five-whys.md` as default root-cause method
- [ ] Cross-reference from PEL `client-engagement/reports/README.md`: weekly + heartbeat templates should reference `5-minto-pyramid.md` as the prose-structure default
...
- [`../../design-guide/thinking-frameworks/README.md`](../../design-guide/thinking-frameworks/README.md) — the new directory's overview
