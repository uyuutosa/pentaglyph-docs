...
│   ├── arc42/                       # internal `01-..-09-` numbering unchanged
...
│   ├── api-contract/
│   ├── detailed-design/
│   ├── diagrams/c4/                 # Structurizr DSL + exports
│   ├── service-design/              # TiSDD: personas / journeys / blueprints
│   ├── templates/
│   ├── user-manual/                 # Diátaxis quadrants
│   ├── impl-plans/                  # volatile (Layer B change-rate)
│   ├── postmortems/                 # volatile
│   ├── reports/                     # volatile
│   ├── cost-estimates/              # volatile
│   └── task-list/                   # volatile
│
├── 02-process/                      # Layer ② Process (formerly design-guide/)
...
│   └── version-control.md
│
├── 04-governance/                   # Layer ④ Governance (formerly governance/)
│   └── …
│
└── 05-measurement/                  # Layer ⑤ Measurement (formerly metrics/, optional)
    └── …
```
...
- Cons:
  ...
  - Reverse-dependency violations look identical to legitimate citations in path strings (`detailed-design/foo.md` ↔ `design-guide/bar.md` — which way is the layer arrow?).
...
- [ ] Land impl-plan `2026-05-16_layer-prefixed-directories-migration.md` (in `docs/impl-plans/`) with phase-by-phase migration steps. The impl-plan currently lives only in the AI-clone downstream subtree; upstream import is tracked as a follow-up PR.
